...
module.exports = function (eleventyConfig) {
  ...
  const { contributor: publicationContributors, contributor_as_it_appears } =
  ...
  return function (params) {
    ...
    const homePageLinkOpenTag = isHomePage
      ? ''
      : `<a class="quire-menu__header__title-link" href="/">`;
    const homePageLinkCloseTag = isHomePage ? '' : `</a>`;

    const contributorContent =
      contributor_as_it_appears ||
      contributors({
        context: publicationContributors,
        format: 'string',
        type: 'primary',
      });
