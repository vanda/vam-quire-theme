...
module.exports = function (eleventyConfig) {
  ...
  return function (params) {
    ...
    const contributorLink = url
      ? link({
          classes: ['quire-contributor__url'],
          name: icon({ type: 'link', description: '' }),
          url,
        })
      : '';
    ...
    const contributorImage = image
      ? html`
          <div class="media-left">
            <img
              class="image quire-contributor__pic"
              src="${path.join(config.figures.imageDir, image)}"
              alt="Picture of ${name}"
            />
          </div>
        `
    ...
    const contributorBio = bio ? html`${markdownify(bio)}` : '';
        ...
        <div class="media">
          <div class="quire-contributor__details media-content">
            ...
            <div class="quire-contributor__bio">
              <div class="has-h5-font-size">
                <span class="quire-contributor__name">${name}</span>
              </div>
              ${contributorBio} ${contributorPagesList()}
            </div>
          </div>
        </div>
      </li>
